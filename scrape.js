...
cachedRequest({url: 'http://www.europarl.europa.eu/delegations/en/home.html'}, function (error, response, html) {
  if (error) {console.log("error:" + error);process.exit(1)}
    ...
    if (Object.keys(r).length < 10) {console.log("can't parse europarl");process.exit(1);}
    file.writeFileSync("./data/delegations.json", JSON.stringify(r));
  }); 

cachedRequest({url: 'http://www.europarl.europa.eu/committees/en/home.html'}, function (error, response, html) {
  if (error) {console.log("error:" + error);process.exit(1)}
    const $ = cheerio.load(html);
    var r={};
